...
public class MergeSort {
    ...
    // Sort a list of caseModel objects based on Assigend lawyer (String) in ascending or descending order
    public List<AdvocateDiaryModel> sortByAssigendLawyer(List<AdvocateDiaryModel> caseList, boolean isDesc) {
        ...
        List<AdvocateDiaryModel> sortedList = new ArrayList<>(caseList);
        mergeSort(sortedList, 0, sortedList.size() - 1, isDesc);
        return sortedList;
    }

    // The mergeSort function recursively splits the list and merges it back
    private void mergeSort(List<AdvocateDiaryModel> list, int left, int right, boolean isDesc) {
        if (left < right) {
    ...
    private void merge(List<AdvocateDiaryModel> list, int left, int mid, int right, boolean isDesc) {
        // Temporary arrays for the two halves
        List<AdvocateDiaryModel> leftList = new ArrayList<>(list.subList(left, mid + 1));
        List<AdvocateDiaryModel> rightList = new ArrayList<>(list.subList(mid + 1, right + 1));
        
        int i = 0, j = 0, k = left;
        
        // Merge the two halves based on Location (String comparison)
        while (i < leftList.size() && j < rightList.size()) {
            if (shouldSwap(leftList.get(i).getAssignedLawyer(), rightList.get(j).getAssignedLawyer(), isDesc)) {
